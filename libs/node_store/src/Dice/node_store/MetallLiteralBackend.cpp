...
namespace Dice::node_store {
	...
	MetallLiteralBackend::MetallLiteralBackend(std::string_view lexical, const rdf4cpp::rdf::storage::node::identifier::NodeID &dataType, std::string_view langTag, metall_manager::allocator_type<std::byte> const &allocator) noexcept
	...
	std::partial_ordering MetallLiteralBackend::operator<=>(const MetallLiteralBackend &other) const noexcept {
		return std::make_tuple(this->datatype_id_, this->lexical_form(), this->language_tag()) <=> std::make_tuple(other.datatype_id_, other.lexical_form(), other.language_tag());
	}

//	bool MetallLiteralBackend::operator==(const MetallLiteralBackend &other) const noexcept {
//		return std::make_tuple(this->datatype_id_, this->lexical_form(), this->language_tag()) == std::make_tuple(other.datatype_id_, other.lexical_form(), other.language_tag());
//	}
	std::partial_ordering MetallLiteralBackend::operator<=>(const MetallLiteralBackend::pointer_t &other) const noexcept {
		if (other)
			return *this <=> *other;
		else
	...
	std::partial_ordering operator<=>(const MetallLiteralBackend::pointer_t &self, const MetallLiteralBackend::pointer_t &other) noexcept {
		return *self <=> *other;
	}
}// namespace Dice::node_store
