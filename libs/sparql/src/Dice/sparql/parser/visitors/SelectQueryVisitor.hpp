...
namespace Dice::sparql::parser::visitors {
	...
	private:
		...
		uint8_t tp_count = 0;
		uint8_t depth = 0;
		std::map<char, std::map<uint8_t, uint8_t>> var_depth_tp;
		std::map<uint8_t, uint8_t> depth_tp;

	public:
		SelectQueryVisitor() = delete;

		explicit SelectQueryVisitor(ParsedSPARQL *q) : query{q} {}
	...
	private:
		...
		void update_odg(rdf4cpp::rdf::query::TriplePattern const &);
	};
