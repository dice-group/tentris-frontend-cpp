...
namespace Dice::sparql::parser::visitors {
	...
	antlrcpp::Any SelectQueryVisitor::visitSelectClause(SparqlParser::SelectClauseContext *ctx) {
		...
		if (ctx->ASTERISK()) {
			for (auto const &var : query->var_to_id) {
				query->projected_variables.push_back(var.first);
			}
		} else {
			for (auto sel_ctx : ctx->selectVariables()) {
				if (sel_ctx->var()) {
					auto var = visitVar(sel_ctx->var()).as<rdf4cpp::rdf::query::Variable>();
					if (not query->var_to_id.contains(var))
						throw std::runtime_error("Variable "+var.name()+" is projected but it does not appear in the WHERE clause.");
					query->projected_variables.push_back(var);
				} else {
	...
	antlrcpp::Any SelectQueryVisitor::visitObjectPath(SparqlParser::ObjectPathContext *ctx) {
		if (ctx->graphNodePath()->varOrTerm()) {
			...
			query->triple_patterns.emplace_back(active_subject, active_predicate, obj);
			update_odg(query->triple_patterns.back());
			tp_count++;
		} else {
	...
	void SelectQueryVisitor::register_var(rdf4cpp::rdf::query::Variable const &var) {
		if (query->var_to_id.contains(var))
			return;
		query->var_to_id[var] = var_id;
		var_id++;
 	}
	...
	void SelectQueryVisitor::update_odg(rdf4cpp::rdf::query::TriplePattern const &tp) {
		...
		for (auto const &node : tp) {
			if (not node.is_variable())
				continue;
			var_ids.push_back(query->var_to_id[rdf4cpp::rdf::query::Variable(node)]);
		}
		query->odg.addOperand(var_ids);
		for (auto const &id : var_ids) {
			if (var_depth_tp.contains(id)) {
				if (var_depth_tp[id].contains(depth)) {
					query->odg.addDependency(tp_count, var_depth_tp[id][depth], id);
