...
RUN apk update && \
    apk add git make cmake boost-build pythonispython3 py3-pip autoconf automake gcc g++ clang clang-dev clang-libs clang-extra-tools clang-static llvm13 llvm13-dev lld pkgconfig libuuid libtool util-linux-dev linux-headers openjdk11-jdk && \
...
RUN git clone --quiet --branch v0.30.10 --depth 1 --recurse-submodules https://gitlab.com/drobilla/serd.git
WORKDIR serd
...
RUN conan user dev -p rUxRq7Z9vBpVgvX -r tentris-private
